...
var parseCommentLines = function(lines, start, end) {
  ...
  lines.map(function(lineObj) {
    ...
    // NOTE: Returning AST for comments disabled until this file is cleaned up.
    // obj['ast'] = lineObj['ast'];
  });
...
/**
 * Start a comment, and traverse until finding another comment.
 */
var getCommentNodeContent = function(ast, node, index) {
  ...
  ast.traverse(function(n, i) {
    // Only care about nodes that start after the comment.
    if (i > index && !foundComment) {
...
var isValidContentCommentNode = function(node, index, fileLevelCommentNodes) {
  ...
  return !isNodeInAST(node, fileLevelCommentNodes) && isContentCommentDelimeter(node, index);
};
...
var getContentCommentNodes = function(ast) {
  ...
  ast.forEach(enums.COMMENT_TYPE, function(node, index) {
    ...
    if (isValidContentCommentNode(node, index, fileLevelCommentNodes)) {
      ...
      nodeClone.ast = getCommentNodeContent(ast, node, index);
      ...
      nodes[commentBlockIndex].push(nodeClone);
...
var getComments = function(ast) {
  ...
  for (var i = 0; i < nodes.length; i++) {
    var json = parseCommentNodes(nodes[i]);
    jsonArray.push(json)
  }
